...
def hash_from_dict(d: Dict, keys: List[str] = None) -> str:
    if keys is not None:
        d = {k: d.get(k) for k in keys}
    return hashlib.sha256(bytes(json.dumps(d, sort_keys=True), 'utf-8')).hexdigest()


def removekeys(d: Dict, keys: List[str]) -> Dict:
    r = dict(d)
    for key in keys:
        try:
            del r[key]
