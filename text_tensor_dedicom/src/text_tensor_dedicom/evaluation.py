...
from text_tensor_dedicom.visualization import heatmap_plot, losses_plot, scatterplot_colored
from text_tensor_dedicom.visualization import fig_to_tensorboard, df_to_txt, df_to_tex, save_fig
from text_tensor_dedicom.models.dimensionality_reduction import umap_transform
    ...
    for topic, words in topics.items():
        if words:
            ...
            for topic_word_ind, topic_word_cos_sim, ind in zip(cos_sims_sorted_indices, cos_sims_sorted, indices):
                topic_column.append(f'{id_to_word[int(ind)]}')
                topic_column.append('(1.0)')
                for idx, cos_sim in zip(topic_word_ind, topic_word_cos_sim):
                    topic_column.append(f'{id_to_word[int(idx)]}')
...
def get_topics(M: np.array,
    ...
    for dim in range(M.shape[1]):
        ...
        if one_topic_per_word:
            row_indices = np.argwhere(M.argmax(axis=1) == dim).flatten()
        else:
            row_indices = np.arange(M.shape[0])
        embeddings = M[row_indices]

        new_sorting_order = np.argsort(embeddings[:, dim], axis=0)[::-1]
        embeddings_sorted = embeddings[new_sorting_order].astype(dtype='float64')
        indices_sorted = row_indices[new_sorting_order]

        counter = 0
        for embedding, id_ in zip(embeddings_sorted, indices_sorted):

            if counter < n:
                topics_paper[f'Topic {dim + 1}'].append(id_to_word[id_])
                if one_topic_per_word:
                    topics_paper[f'Topic {dim + 1}'].append(f'({np.round(embedding.max(), 3)})')
                else:
                    topics_paper[f'Topic {dim + 1}'].append(f'({np.round(embedding[dim], 3)})')

            if counter >= n and not save_dir:
                break
            topics_all[f'Topic {dim + 1}'].append((id_to_word[id_], id_, embedding))
...
def evaluate_topic_sparsity(M: np.array,
    ...
    topics, _ = get_topics(M, n, id_to_word)
...
def evaluate(M: np.array,
             ...
             one_topic_per_word: bool = False,
             bin_names: Optional[List[str]] = None) -> Tuple[Dict, Dict]:
    ...
    if writer:
        heatmap = fig_to_tensorboard(heatmap_plot(R, bin_names=bin_names))
        topic_text = '=' * 64 + '\n\n'
        for i, topic in enumerate(topics.values(), 1):
            topic_count = topic_counts.get(f'Topic {i}', 0)
            topic_text += f'TOPIC {i} ({topic_count}): '
...
def generate_paper_output(save_dir: str,
                          ...
                          R: Optional[np.array] = None,
                          one_topic_per_word: bool = True,
                          bin_names: Optional[List[str]] = None,
                          writer=None):

    json.dump(id_to_word, open(os.path.join(save_dir, 'id_to_word.json'), 'w'))
    ...
    if writer:
        ...
        for i, topic in enumerate(topics.values(), 1):
            ...
            topic_text += f'TOPIC {i} ({topic_count}): '
            topic_text += '[' + ' '.join(topic) + ' ]\n\n'
        writer.add_text('topics', topic_text, 10000)

    # Save Topic Table
    df_to_tex(df=pd.DataFrame.from_dict(topics),
              save_dir=os.path.join(save_dir, 'paper_output'),
              save_name=f'tab_topics_{method}.tex',
              caption=f'Each column lists the top 10 representative words per dimension of the basis matrix $A$.',
              label=f'topics_{method}')
    ...
    df_to_tex(df=pd.DataFrame(topic_counts, index=[0]),
              save_dir=os.path.join(save_dir, 'paper_output'),
              save_name=f'tab_topic_counts_{method}.tex',
              caption=f'Amount of words assigned to each topic, after training.',
              label=f'topic_counts_{method}')
    df_to_txt(df=pd.DataFrame(topic_counts, index=[0]),
              save_dir=os.path.join(save_dir, 'output'),
              save_name=f'tab_topic_counts_{method}.txt')

    # Save Similar Words Table
    df_to_tex(df=pd.DataFrame.from_dict(similar_words),
              save_dir=os.path.join(save_dir, 'paper_output'),
              save_name=f'tab_similar_words_{method}.tex',
              caption=f'For the most significant two words per topic, '
              f'the four nearest neighbors based on cosine similarity are listed.',
              label=f'similar_words_{method}')
    ...
    fig_umap_titles = scatterplot_colored(M_umap, titles=bin_names, word_to_titles=word_to_titles, id_to_word=id_to_word)
    save_fig(fig_umap_titles,
             save_dir=save_dir,
             save_name=f'scatterplot_titles_umap_{method}')
    plt.close(fig_umap_titles)
    if writer is not None:
        writer.add_image('umap_titles', fig_to_tensorboard(fig_umap_titles), 0)

    fig_umap_topics = scatterplot_colored(M_umap, topics_all=topics_all, id_to_word=id_to_word)
    save_fig(fig_umap_topics,
             save_dir=save_dir,
             save_name=f'scatterplot_topics_umap_{method}')
    plt.close(fig_umap_topics)
    if writer is not None:
        writer.add_image('umap_topics', fig_to_tensorboard(fig_umap_topics), 0)
    ...
    if R is not None:
        fig_heatmap = heatmap_plot(R=R, bin_names=bin_names)
